#line 1 "C:\\Users\\emanu\\OneDrive\\Documenti\\CamplusProgettoLibreriaTFTDebug1\\CamplusUI-TFT-Documentato\\src\\Button.cpp"
#include "Button.h"
#include "Container.h"
#include <Arduino.h>
    ...
    if(!hidden) 
    {
        ...
        switch(buttonShape) {
            case Shape::Rectangular:
                container->myLCD->fillRect(X, Y, Width, Height, Color); // Uses current library for TFT
                break;
            
            case Shape::Circular:
                container->myLCD->fillCircle((int16_t)(X+Width/2), (int16_t)(Y+Height/2), (int16_t)Width/2, (int16_t)Height/2);
                break;
        ...
        // Tries to align to center the text, textSize in pixel is calculated by multipying by 10 textSize. If textSize is bigger than width, cursorX will be equal to X to maximize space available
        if(textSize * 10 * (text.length() - 2) <= Width) {
            cursorX = (int16_t)(X + (Width - textSize * 10 * (text.length() - 2)) / 2);
        }
        ...
        container->myLCD->setTextColor(textColor);
        container->myLCD->setTextSize(textSize);
        container->myLCD->setCursor(cursorX, cursorY);
        // Prints button text
        container->myLCD->println(text);
    }
